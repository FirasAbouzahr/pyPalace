...
* **Elements** (output from pyPalace.Boundaries.Elements) - Used to define multielement lumped ports.

```python
Elements(Attributes,Direction,CoordinateSystem):
```
...
* **Samples** (output from pyPalace.Solver.Driven_Samples) - Specifies additional frequency sweep options.
...
```python
pyPalace.Simulation(path_to_palace,path_to_json)
```
Used to create a Simulation object to run AWS Palace simulations directly in python and analyze results automatically for calculations relevant to superconducting quantum circuits.

* **path_to_palace** (string) - Path to local or high-performance computing palace installation. 
* **path_to_json** (integer) - Path to AWS Palace config file.

```python
Simulation.run_palace(n)
```
Run AWS Palace simulation from the specified config file in pyPalace.Simulation, only for local simulations.  

* **n** (string) - Number of MPI processes to run your simulation with. 

```python
Simulation.run_palace_HPC(n,HPC_options,custom_script_name = None)
```
Run AWS Palace simulation from the specified config file in pyPalace.Simulation on a slurm-managed HPC. Generate a slurm script and subsequently submit job. 

* **n** (string) - Number of MPI processes to run your simulation with.
* **HPC_options** (output from pyPalace.Simulation.HPC_options) - User specified slurm directives.
* **custom_script_name** (string) - Custom name for the generated slurm script.

### Module-level API

These functions can be called directly without creating a Simulation object.

```python
Simulation.HPC_options(partition,time,nodes,ntasks_per_node,mem,job_name,custom = None)
```
